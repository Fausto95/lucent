...
export const explanations: { code: string; title: string; summary: string; details: string; fix: string; wrong: Record<string, string>; right: Record<string, string> }[] = [
  {
    "code": "LUCENT1001",
  ...
  },
  {
    ...
    "details": "Errors cross into JavaScript as `Error` objects with a name, a message and a stack that points at the Lucent source. A thrown string or number has none of that and no native type to catch it by.",
    "fix": "throw `new Error(…)`, a built-in error such as `TypeError`, or a class that extends `Error`",
    "wrong": {
      "example.lucent.ts": "export function port(text: string): number {\n  const n = Number(text);\n  if (!Number.isInteger(n)) throw \"not a port\";\n  return n;\n}\n"
    },
    "right": {
  ...
  },
  {
    ...
    "wrong": {
      "example.lucent.ts": "function tick(): number {\n  return 1;\n}\nexport function reset(): void {\n  // @ts-expect-error: assigning a function\n  tick = () => 0;\n}\n"
    },
    "right": {
      "example.lucent.ts": "let tick = (): number => 1;\nexport function reset(): void {\n  tick = () => 0;\n}\nexport function now(): number {\n  return tick();\n}\n"
    }
  },
  {
    "code": "LUCENT1009",
  ...
  },
  {
    ...
    "details": "Native code needs to know each value's layout at compile time. `any` gives it none, and `unknown` only stands for a caught error, whose type JavaScript does not say.",
    "fix": "use a concrete type, a union, or a generic parameter",
    "wrong": {
      "example.lucent.ts": "export function size(value: any): number {\n  return value.length;\n}\n"
    },
    "right": {
      "example.lucent.ts": "export function size(value: string): number {\n  return value.length;\n}\n"
    }
  },
  {
    "code": "LUCENT2002",
    ...
    "right": {
      "example.lucent.ts": "type Person = { name: string; age: number };\nexport function label(p: Person): string {\n  return `${p.name} (${p.age})`;\n}\n"
    }
  },
  {
    "code": "LUCENT2003",
  ...
  },
  {
    ...
    "details": "A value from JavaScript carries no type, so Lucent decides which union member it is from the value itself. Object members need a string-literal field, such as `kind`, whose value names the member.",
    "fix": "add a string-literal field such as `kind: \"circle\"` to each object member",
    "wrong": {
      "example.lucent.ts": "export function area(shape: { radius: number } | { side: number }): number {\n  return \"radius\" in shape ? Math.PI * shape.radius ** 2 : shape.side ** 2;\n}\n"
    },
    "right": {
      "example.lucent.ts": "export function area(shape: { kind: \"circle\"; radius: number } | { kind: \"square\"; side: number }): number {\n  return shape.kind === \"circle\" ? Math.PI * shape.radius ** 2 : shape.side ** 2;\n}\n"
    }
  },
  {
    "code": "LUCENT2006",
    ...
    "right": {
      "example.lucent.ts": "function* count(n: number): Generator<number> {\n  for (let i = 0; i < n; i++) yield i;\n}\nexport function counted(n: number): number[] {\n  return [...count(n)];\n}\n"
    }
  },
  {
    "code": "LUCENT2007",
    ...
    "details": "Generics compile to C++ templates, instantiated for the types the module uses. JavaScript calls an export without types, so there is no instantiation to call.",
    "fix": "keep the generic function private and export concrete wrappers",
    "wrong": {
      "example.lucent.ts": "export function last<T>(items: T[]): T | undefined {\n  return items[items.length - 1];\n}\n"
    },
    "right": {
      "example.lucent.ts": "function last<T>(items: T[]): T | undefined {\n  return items[items.length - 1];\n}\nexport function lastName(names: string[]): string | undefined {\n  return last(names);\n}\n"
    }
  },
  {
    "code": "LUCENT2008",
    ...
    "details": "An interface with methods compiles to an abstract C++ base class, and only classes that declare `implements` derive from it. A class that matches the interface only by shape, or an object literal, is not one of them.",
    "fix": "add `implements` to the class",
    "wrong": {
      "example.lucent.ts": "interface Shape { area(): number }\nclass Square {\n  constructor(readonly side: number) {}\n  area(): number {\n    return this.side ** 2;\n  }\n}\nexport function area(): number {\n  const s: Shape = new Square(2);\n  return s.area();\n}\n"
    },
    "right": {
  ...
  },
  {
    ...
    "details": "The implementing method overrides the interface's native method, so it needs the same parameter and result types. TypeScript accepts compatible variations (an optional parameter, a narrower result) that are different native signatures.",
    "fix": "declare the method with exactly the interface's parameter and result types",
    "wrong": {
      "example.lucent.ts": "interface Store { get(key: string): string | undefined }\nclass Memory implements Store {\n  get(key: string): string {\n    return key;\n  }\n}\nexport function read(): string | undefined {\n  const s: Store = new Memory();\n  return s.get(\"a\");\n}\n"
    },
    "right": {
  ...
  },
  {
    ...
    "details": "Everything a module runs is compiled to native code, so it can only use other Lucent modules and the built-in `lucent:` modules (`lucent:core`, `lucent:thread`, `lucent:platform`, the SDKs). An npm package or a plain `.ts` file has no native implementation.",
    ...
    "right": {
      "example.lucent.ts": "import { now } from \"lucent:core\";\nexport function stamp(): number {\n  return now();\n}\n"
    }
  },
  {
    "code": "LUCENT3002",
    ...
    "details": "A module's top level holds declarations only: functions, classes, types and variables. Statements that run when the module loads have no native place to run, since native modules are created on first use.",
    "fix": "move the statement into a function, or into a variable's initializer",
    "wrong": {
      "example.lucent.ts": "const cache = new Map<string, number>();\ncache.set(\"zero\", 0);\nexport function lookup(k: string): number | undefined {\n  return cache.get(k);\n}\n"
    },
    "right": {
      "example.lucent.ts": "const cache = new Map<string, number>([[\"zero\", 0]]);\nexport function lookup(k: string): number | undefined {\n  return cache.get(k);\n}\n"
    }
  },
  {
    "code": "LUCENT3003",
    ...
    "right": {
      "example.lucent.ts": "export function twice(n: number): number {\n  return n * 2;\n}\n"
    }
  },
  {
    "code": "LUCENT3004",
    ...
    "details": "`lucent:ios/*` modules exist on iOS and `lucent:android/*` on Android. A shared module uses them inside `if (PLATFORM === \"ios\")` (or the Android branch), and a `*.ios.lucent.ts` file only its own platform's.",
    "fix": "use the SDK inside a PLATFORM branch, or in the platform's own file",
    "wrong": {
      "example.lucent.ts": "import { UIDevice } from \"lucent:ios/UIKit\";\nimport { main } from \"lucent:thread\";\nexport async function model(): Promise<string> {\n  return main(() => UIDevice.current.model);\n}\n"
    },
    "right": {
