...
export function SiteLayout() {
  ...
  const isGetStarted = useLocation({ select: (location) => location.pathname.startsWith("/get-started") });
  useEffect(() => {
    document.title = isLanguage
      ? "Language — Lucent"
      : isGetStarted
        ? "Get started — Lucent"
      ...
      <div {...stylex.props(styles.pageShell)}>
        <header {...stylex.props(styles.header)}>
          ...
          <nav aria-label="Main navigation" {...stylex.props(styles.mainNav)}>
            ...
            <Link to="/get-started/" {...stylex.props(styles.languageNavLink, isGetStarted && styles.activeNav)}>
              {"Get started"}
            </Link>
