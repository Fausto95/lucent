...
export const blocks: Block[] = [
  {
    ...
    rows: [
      ...
      [
        ...
        "`lucent::String`, UTF-16",
        "a copy",
        "a string; any string, for literal types",
      ],
      [
        "`T | null`, `T | undefined`, `x?: T`",
        "`lucent::Opt<T>`",
        "the value, or its absence",
        "`null` or `undefined` for absent: both, whatever the type declares",
      ],
      ["`T[]`", "`lucent::Array<T>`", "a copy", "an array; each element is checked"],
      ["`[A, B]`", "`std::tuple<A, B>`", "a copy", "an array"],
      [
        "object types, interfaces without methods",
        "a struct, by shape",
        "a copy",
        "an object with each declared field; extra fields are ignored",
      ],
      ...
      [
        "`RegExp`",
        "`lucent::RegExp`",
        "its source and flags",
        "a `RegExp`; `lastIndex` isn't kept",
      ],
      [
        "enums",
        "`double` or `lucent::String`",
        "a copy",
        "a number or a string; membership isn't checked",
      ],
      [
        "unions",
        "`std::variant`",
        "the member JavaScript's value matches",
        "a value of one member; object members need a string-literal discriminant (`LUCENT2005`)",
      ],
      [
        "classes",
        "a reference-counted object",
        "**a reference**, with its identity",
        "an instance of that Lucent class",
      ],
      [
        "interfaces a class `implements`",
        "the class's object",
        "**a reference**",
        "an instance of a Lucent class that implements it",
      ],
      ...
      [
        ...
        "`lucent::Promise<T>`",
        "**a promise**",
        "a promise or any value, as `await` takes",
      ],
      ["`AbortSignal`", "`lucent::AbortSignal`", "into Lucent only", "an `AbortSignal`"],
      [
        "`Error`, `TypeError`, `RangeError`",
        "`lucent::Error`",
        "its name, message and code",
        "any value",
      ],
      [
        "`Iterable<T>` (parameters only)",
        "`lucent::Iter<T>`",
        "a snapshot, as `Array.from` takes",
        "an iterable",
      ],
      ["`void`", "`void`", "`undefined`", ""],
    ],
