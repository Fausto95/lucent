  ...
  blocks: [
    ...
    { kind: "h2", text: "The model" },
    {
      ...
      items: [
        ...
        "Rendering is synchronous. Mutation, async calls and loops in the render body are rejected. Event handlers may update view state.",
        ...
        "`resource(() => …)` owns a closeable value for the view lifetime. Sync or async `effect(() => …)` runs after render; cleanup runs on teardown.",
        "Props are strings, numbers, booleans, arrays of those, nullable scalars, and `Event<void | string | boolean | number>` callbacks.",
        "`condition ? <A /> : <B />` chooses a view. `For` (optionally keyed) repeats one row for each element of a string, number, or boolean array.",
        "Compose other `.lucent.tsx` components by importing them; pass typed props.",
        "Not in the subset: hooks, arbitrary React components, lazy virtualized lists, JSX spreads and fragments, custom `children`.",
      ],
    },
