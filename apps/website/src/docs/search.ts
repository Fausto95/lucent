...
const hits = (field: string[], term: string): number =>
  field.filter((w) => w.startsWith(term)).length;
...
function snippetOf(text: string, terms: string[]): string {
  ...
  const at = Math.min(
    ...terms
      .map((t) =>
        lower.search(
          new RegExp(`(^|[^\\p{L}\\p{N}_])${t.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`, "u"),
        ),
      )
      .filter((i) => i >= 0),
    text.length,
...
export function search(entries: SearchEntry[], query: string, limit = 12): SearchResult[] {
  ...
  const scored = entries.flatMap((entry) => {
    const fields = {
      page: words(entry.page),
      heading: words(entry.heading ?? ""),
      text: words(entry.text),
    };
    let score = 0;
    for (const term of terms) {
      const found = (Object.keys(WEIGHTS) as (keyof typeof WEIGHTS)[]).reduce(
        (sum, key) => sum + Math.min(hits(fields[key], term), 3) * WEIGHTS[key],
        0,
      );
      if (found === 0) return [];
      score += found;
