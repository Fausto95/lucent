...
export default function run(mod, print, lucentClass, mods) {
  ...
  print(
    JSON.stringify(mod.bits(12, 10)),
    JSON.stringify(mod.bits(-5, 3)),
    JSON.stringify(mod.bits(2147483647, 1)),
  );
  print(
    JSON.stringify(mod.parse(["42", " 3.5 ", "", "0x10", "1e3", "abc", "-0", "Infinity", "12px"])),
  );
  ...
  print(
    JSON.stringify(mod.checks(3)),
    JSON.stringify(mod.checks(3.5)),
  ...
  print(
    ...
    mod.multiplyAdd(1 / 3, 3, -1),
    mod.multiplyAdd(1e308, 10, -1e308),
  );
