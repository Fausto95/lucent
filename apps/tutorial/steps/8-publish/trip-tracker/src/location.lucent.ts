...
// Kept while the permission prompt is on screen.
let prompter: CLLocationManager | null = null;
...
/** Shows the system's permission prompt on iOS. Android asks from JavaScript, with PermissionsAndroid. */
export async function requestPermission(): Promise<void> {
  if (PLATFORM === "ios") {
    await main(() => {
      prompter = new CLLocationManager();
      prompter.requestWhenInUseAuthorization();
    });
  }
}
