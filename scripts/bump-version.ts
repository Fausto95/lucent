...
const manifests = dirs
  .map((f) => ({ f, pkg: JSON.parse(fs.readFileSync(f, "utf8")) }))
  .filter((m) => !m.pkg.private && m.pkg.name?.startsWith("@lucent-lang/"));
const current = manifests[0]!.pkg.version as string;
const [major, minor, patch] = current.split("-")[0]!.split(".").map(Number) as [
  number,
  number,
  number,
];
