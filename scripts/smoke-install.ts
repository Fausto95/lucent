...
for (const p of packages) {
  const out = sh("pnpm", ["pack", "--pack-destination", work], path.join(root, "packages", p))
    .trim()
    .split("\n")
    .pop()!;
  const name = JSON.parse(fs.readFileSync(path.join(root, "packages", p, "package.json"), "utf8"))
    .name as string;
...
const haptics = sh(
  ...
  path.join(root, "examples/lucent-haptics"),
)
  .trim()
  .split("\n")
  .pop()!;
...
fs.writeFileSync(
  ...
  JSON.stringify({ name: "smoke", private: true, dependencies: tarballs }, null, 2),
);
sh("npm", ["install", "--no-audit", "--no-fund", "--loglevel=error"], app);
sh(
  "npm",
  ["install", "--no-audit", "--no-fund", "--loglevel=error", "--save-dev", "typescript@~5.9.3"],
  app,
);

const installed = path.join(app, "node_modules/@lucent-lang");
const lucentPackages = fs.readdirSync(installed).sort();
if (lucentPackages.join() !== "lucent")
  throw new Error(`expected @lucent-lang/lucent only, got ${lucentPackages.join(", ")}`);
if (fs.existsSync(path.join(installed, "lucent/src")))
  throw new Error("lucent/src should not be published");
...
if (!fs.readFileSync(path.join(app, "metro.config.js"), "utf8").includes("withLucent("))
  throw new Error("init did not wrap the Metro config");
if (
  !fs
    .readFileSync(path.join(app, "android/app/build.gradle"), "utf8")
    .includes("gradle/lucent.gradle")
)
  throw new Error("init did not apply the Gradle task");
// The line init applies finds the task in the installed package.
if (!fs.existsSync(path.join(app, "node_modules/@lucent-lang/lucent/gradle/lucent.gradle")))
  throw new Error("the Gradle task is not in the package");
...
const generated = {
  ios: ["cpp/generated/ios/m_hello.cpp", "cpp/generated/ios/m_device.mm"],
  android: ["cpp/generated/android/m_hello.cpp", "cpp/generated/android/m_device.cpp"],
};
for (const f of [
...
sh(
  ...
  [
    ...
    "-fsyntax-only",
    `-I${native}/cpp`,
    `-I${native}/cpp/generated/${targets[0]}`,
    path.join(native, `cpp/generated/${targets[0]}/m_hello.cpp`),
  ],
  app,
);
...
sh(
  ...
  [
    "-e",
    'require("@lucent-lang/lucent/metro").withLucent({ transformer: { babelTransformerPath: "metro-babel-transformer" } }); if (typeof require("@lucent-lang/lucent/app.plugin.js") !== "function") process.exit(1)',
  ],
  app,
  { LUCENT_WATCH: "0" },
);
...
sh(
  ...
  [
    "-e",
    'const core = require("@lucent-lang/lucent/core"); if (core.errorCode(core.error("E_X", "x")) !== "E_X") process.exit(1); core.delay(1).then(() => process.exit(0), () => process.exit(1))',
  ],
  app,
);
...
if (!codes.includes(1001))
  throw new Error(`expected LUCENT1001 from the plugin, got ${JSON.stringify(codes)}`);
fs.rmSync(bad);
...
async function editorDiagnostics(file: string): Promise<number[]> {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 100; attempt++) {
      const diags = (await request("semanticDiagnosticsSync", { file })) as {
        code: number;
        source?: string;
      }[];
