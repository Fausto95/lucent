...
export function compileSamples(app: string, samples: Sample[]): { diagnostics: Diagnostic[]; files: Map<string, string>; proxies: Map<string, string> } {
  ...
  return { diagnostics: result.diagnostics.map((d) => ({ ...d, file: d.file && path.relative(dir, d.file) })), files, proxies: result.proxies };
}
