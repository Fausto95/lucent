...
const generated: Record<string, string> = {
  ...
  // The CLI reference is generated from the command table the CLI parses with.
  "cli.ts": `${header}export const cliCommands: { name: string; summary: string; flags: { flag: string; description: string }[] }[] = ${JSON.stringify(
    commands.map((c) => ({ name: c.name, summary: c.summary, flags: c.flags.map((f) => ({ flag: `--${f.name}${f.value ? (f.optional ? ` [<${f.value}>]` : ` <${f.value}>`) : ""}`, description: f.description })) })),
    null,
    2,
  )};\n\nexport const globalFlags: { flag: string; description: string }[] = ${JSON.stringify(
    GLOBAL_FLAGS.map((f) => ({ flag: `--${f.name}${f.value ? ` <${f.value}>` : ""}`, description: f.description })),
    null,
    2,
  )};\n`,
