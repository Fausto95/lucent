...
describe("lucent build", () => {
  ...
  test("reports diagnostics with codeframes and writes nothing", async () => {
    ...
    expect(result.diagnostics[0]!.rendered).toContain("LUCENT1004");
...
describe("project imports", () => {
  test("rebuilds importers when a dependency changes", async () => {
    ...
    expect(next.cached).toContain("src/native/text.lucent.ts");
  });
  test("discovers tsx files", () => {
    const root = project();
...
test("preserves warnings on successful incremental builds", async () => {
  ...
  const first = await build({ root, host: "expo" });
  expect(first.ok).toBe(true);
  expect(first.diagnostics.some((d) => d.severity === "warning" && d.rendered.includes("LUCENT3002"))).toBe(true);
  const next = await build({ root, host: "expo" });
  expect(next.ok).toBe(true);
