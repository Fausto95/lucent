...
test("shares a class identity across imported modules", () => {
  ...
  const b = compile('import {Counter} from "./counter.lucent"; export function f():number { const c = new Counter(1); return c.increment(2); }', {fileName:"use.lucent.ts", sources:{"counter.lucent.ts":counter}});
...
test("requires initialized fields and rejects async shared methods", () => {
  expect(compile(counter.replace('value: number = 0', 'value: number'), {fileName:"counter.lucent.ts"}).diagnostics.length).toBeGreaterThan(0);
  expect(compile(counter.replace('increment(delta: number): number', 'async increment(delta: number): Promise<number>'), {fileName:"counter.lucent.ts"}).diagnostics.length).toBeGreaterThan(0);
});
