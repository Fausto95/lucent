...
describe("Lucent module imports", () => {
  ...
  test("links aliased functions and types across ts and tsx files", () => {
    const result = compile(`import { double as scale, type Point } from "./lib/math.lucent";
  ...
  test("rejects missing exports and arbitrary JS dependencies", () => {
    for (const source of ["./lib/missing.lucent", "react"]) {
      expect(compile(`import { double } from "${source}";`, { fileName: "main.lucent.ts", sources }).diagnostics[0]?.code).toBe("NT1006");
    }
    expect(compile('import { twice } from "./lib/math.lucent";', { fileName: "main.lucent.ts", sources }).diagnostics[0]?.code).toBe("NT1006");
  });
  ...
  test("rejects a type-only import used as a value", () => {
    const result = compile('import type { double } from "./lib/math.lucent"; export function f(): number { return double(1); }', {fileName: "main.lucent.ts", sources});
    expect(result.diagnostics.length).toBeGreaterThan(0);
  });
  test("does not expose dependency private names to the importer", () => {
    const result = compile('import { double } from "./lib/math.lucent"; export function f(): number { return twice(1); }', {fileName: "main.lucent.ts", sources});
