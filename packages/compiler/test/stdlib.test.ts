...
test.each([
 ...
 ['filesystem','export async function f(path:string):Promise<Uint8Array>{return await read(path);}','read',['filesystem']],
 ['filesystem','export async function f(path:string,bytes:Uint8Array):Promise<void>{await write(path,bytes);}','write',['filesystem']],
 ...
 expect(r.diagnostics).toEqual([]);expect(r.module?.capabilities??[]).toEqual(capabilities);
 expect(r.module?.functions.some(f=>f.binding)).toBe(true);
});
test('warns when MainThread calls a CPU-intensive native binding',()=>{
 const r=compile('import {sha256} from "@lucent-lang/crypto"; @MainThread export async function hash(bytes:Uint8Array):Promise<string>{return sha256(bytes);}',{fileName:'hash.lucent.ts'});
