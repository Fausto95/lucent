...
function app(): string {
  ...
  write("node_modules/lucent-b/lib/storage.lucent.ts", "export function where(): string { return 'b'; }\n");
  write("node_modules/plain-js/package.json", JSON.stringify({ name: "plain-js", version: "1.0.0" }));
...
describe("Lucent packages", () => {
  it("names a package's modules <package>/<module>, and the app's by their file", () => {
    ...
    expect(moduleNameOf(path.join(root, "node_modules/lucent-a/src/storage.lucent.ts"))).toBe("lucent-a/storage");
    expect(moduleNameOf(path.join(root, "node_modules/lucent-a/src/nested/deep.lucent.ts"))).toBe("lucent-a/nested/deep");
    expect(moduleNameOf(path.join(root, "node_modules/lucent-b/lib/storage.lucent.ts"))).toBe("lucent-b/storage");
  });
  ...
  it("compiles modules of the same name from different packages side by side", () => {
    ...
    expect([...r.proxies.keys()].sort()).toEqual(["lucent-a/nested/deep", "lucent-a/storage", "lucent-b/storage", "storage"]);
