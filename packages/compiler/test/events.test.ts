...
test("type checks native event declarations and emission", () => {
  ...
  expect(compile(source.replace("percent: 50", 'percent: "bad"'), {fileName:"progress.lucent.ts"}).diagnostics.length).toBeGreaterThan(0);
});
test("native views accept typed event props", () => {
  const result = compile(`import { Button, type NativeView } from "@lucent-lang/ui";
    import type { Event } from "@lucent-lang/events";
    type Props = { onPress: Event<void> };
    export function Action(props: Props): NativeView { return <Button title="Run" onPress={props.onPress} />; }`, {fileName:"action.lucent.tsx"});
  expect(result.diagnostics).toEqual([]);
});
