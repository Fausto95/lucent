...
test("rejects unguarded platform-specific calls", () => {
  expect(check('export function f():number{return iosOnly();}').diagnostics).toEqual(expect.arrayContaining([expect.objectContaining({code:"NT2004"})]));
});
test.each([
...
test("rejects an exported helper reachable without a guard", () => {
  expect(check('export function f():number{if(Platform.OS === "ios"){return helper();}return 0;} export function helper():number{return iosOnly();}').module).toBeNull();
});
