...
describe("discriminated unions", () => {
  ...
  ])("rejects invalid union operations: %s", source => { expect(run(source).diagnostics.length).toBeGreaterThan(0); });
  test("narrows the remaining branch after early return", () => {
    expect(run('export function f(s: State): number { if (s.kind === "idle") { return 0; } return s.value; }').diagnostics).toEqual([]);
  });
  test("rejects duplicate tags", () => {
    expect(compile('type Bad = { kind: "same"; a: number } | { kind: "same"; b: string };', {fileName: "bad.lucent.ts"}).diagnostics.length).toBeGreaterThan(0);
  });
});
