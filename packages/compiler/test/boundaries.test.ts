...
test("rejects unsupported event and shared object boundaries before code generation", () => {
  const sources = [
    'import {event} from "@lucent-lang/events"; export const e=event<Uint8Array>();',
    'import type {Event} from "@lucent-lang/events"; export function f(callback:Event<void>):void {}',
  ...
  for (const source of sources) expect(compile(source,{fileName:"invalid.lucent.ts"}).diagnostics.length,source).toBeGreaterThan(0);
});
