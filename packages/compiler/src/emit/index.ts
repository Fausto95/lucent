...
export function emitProgram(lp: LucentProgram): EmitResult {
  ...
  const ifaceFwd = [...ctx.reg.ifaces.values()].map((i) => `struct ${i.cppName};`);
  const ifaceDefs = [...ctx.reg.ifaces.values()].map((i) => ctx.guard(() => emitIface(ctx, i))).filter((d): d is string => d !== undefined);
