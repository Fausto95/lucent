...
import { fromObjc, noteFramework, objcTypeName, type SdkClassRef, sdkInterfacesOf, toObjcCode } from "./native.ts";
 ...
 */
export function objcDelegate(ctx: Ctx, module: LucentModule, info: ClassInfo): { lines: string; decl: string } | undefined {
  ...
  if (info.typeParams.length) fail(info.decl, Codes.UnsupportedClassFeature, "generic classes cannot implement SDK protocols");
  ...
  for (const p of protocols) {
    for (const m of p.cls.methods ?? []) {
      ...
      const impl = info.decl.members.find((x): x is ts.MethodDeclaration => ts.isMethodDeclaration(x) && memberName(x) === m.name && !!x.body);
 ...
 */
function forward(em: FnEmitter, info: ClassInfo, p: SdkClassRef, m: SdkMethodSchema, impl: ts.MethodDeclaration): string {
  ...
  const call = `s_->${cppIdent(m.name)}(${names.slice(0, n).map((a, i) => fromObjc(em, a, params[i]!, fn.params[i]!, what).c).join(", ")})`;
  ...
  if (!isVoid) {
    const r = ret.nullable ? `lucent::objc::ifPresent(r_, [&](const auto& x_) { return ${toObjcCode({ ...ret, nullable: false } as SdkType, "x_", false)}; })` : toObjcCode(ret, "r_", false);
    return `${head} { auto s_ = self->self_; return lucent::callNow([&]() -> ${objcTypeName(ret)} { auto r_ = ${call}; return ${r}; }); }`;
  }
  if (p.cls.mainActor || m.mainActor) return `${head} { auto s_ = self->self_; lucent::callNow([&]() { (void)${call}; }); }`;
