...
export class BindingsEmitter {
  ...
  generate(mods: ModuleExports[], initOrder: LucentModule[]): string {
    ...
    for (const id of this.classes)
      out.push(
        `void proto_${this.reg.cls(id).cppName}(jsi::Runtime& rt, Host& host, jsi::Object& proto);`,
      );
    const specs: string[] = [];
  ...
  private structConvert(id: string): string {
    ...
    info.fields.forEach((f, i) => {
      ...
      from.push(
        `  out->${field} = Convert<${ft}>::fromJs(rt, o.getProperty(rt, h.prop(rt, n${i})), p.field(${q(f.name)}));`,
      );
      if (f.type.k === "opt")
        to.push(
          `  if (!v->${field}.isUndefined()) o.setProperty(rt, h.prop(rt, n${i}), Convert<${ft}>::toJs(rt, h, v->${field}));`,
        );
      else
        to.push(
          `  o.setProperty(rt, h.prop(rt, n${i}), Convert<${ft}>::toJs(rt, h, v->${field}));`,
        );
    });
    return [
  ...
  private classConvert(id: string): string {
    ...
    return [
      ...
      ...this.reg
        ...
        .map(
          (d) =>
            `  if (auto d = std::dynamic_pointer_cast<lucent_app::${d.cppName}>(v)) return Convert<${this.reg.cpp({ k: "class", id: d.id, args: [] })}>::toJs(rt, h, d);`,
        ),
  ...
  private prototype(info: ClassInfo): string {
    ...
    const lines: string[] = [
      `void proto_${info.cppName}(jsi::Runtime& rt, Host& host, jsi::Object& proto) {`,
    ];
    for (const m of publicMembers(this.ctx, info)) {
      if (m.kind === "method") {
        const d = m.node as ts.MethodDeclaration;
        const fname = `${name}.${memberName(d)}`;
        const body = this.callBody(
          fname,
          m.params!,
          m.ret!,
          m.async!,
          `self->${cppIdent(memberName(d))}`,
          `auto self = Convert<${selfT}>::fromJs(rt, thisVal, Path{${q(fname)}, "this"});`,
          ["self"],
        );
        lines.push(
          `  defineFunction(rt, proto, ${q(memberName(d))}, ${m.params!.length}, [installed = host.shared_from_this()](jsi::Runtime& rt, const jsi::Value& thisVal, const jsi::Value* args, size_t count) -> jsi::Value {\n${body}\n  });`,
        );
      } else {
        const fname = `${name}.${m.name}`;
        const t = this.reg.cpp(m.types[0]!);
        const getExpr =
          m.kind === "accessor" ? `self->get_${cppIdent(m.name)}()` : `self->${cppIdent(m.name)}`;
        ...
        if (m.writable) {
          const assign =
            m.kind === "accessor"
              ? `self->set_${cppIdent(m.name)}(value)`
              : `self->${cppIdent(m.name)} = value`;
    ...
    if (info.base) {
      ...
      lines.push(
        `  jsi::Object& baseProto = host.prototype(rt, ${q(base.id)}, proto_${base.cppName});`,
      );
      lines.push(
        `  rt.global().getPropertyAsObject(rt, "Object").getPropertyAsFunction(rt, "setPrototypeOf").call(rt, proto, baseProto);`,
      );
    }
    lines.push("}");
    return lines.join("\n");
  ...
  private callBody(
    ...
    params.forEach((p, i) => {
      ...
      if (p.rest) {
        ...
        conv.push(
          `${t} ${n}; for (size_t i = ${i}; i < count; i++) ${n}.push(Convert<${this.reg.cpp(elem)}>::fromJs(rt, args[i], Path{${q(fname)}, "argument " + std::to_string(i + 1)}));`,
        );
      } else {
        conv.push(
          `auto ${n} = Convert<${t}>::fromJs(rt, arg(args, count, ${i}), Path{${q(fname)}, ${q(`argument '${p.name}'`)}});`,
        );
      }
    });
  ...
  private installer(m: ModuleExports): string {
    ...
    const lines: string[] = [
      `void install_${m.module.ns}(jsi::Runtime& rt, Host& host, jsi::Object& exports) {`,
    ];
    for (const f of m.functions) {
      ...
      lines.push(
        `  defineFunction(rt, exports, ${q(name)}, ${f.params.length}, [installed = host.shared_from_this()](jsi::Runtime& rt, const jsi::Value&, const jsi::Value* args, size_t count) -> jsi::Value {\n${body}\n  });`,
      );
    }
    for (const c of m.classes) {
      ...
      lines.push(
        `  defineClass(rt, host, exports, ${q(name)}, ${q(c.id)}, proto_${c.cppName}, ${params.length}, [installed = host.shared_from_this()](jsi::Runtime& rt, const jsi::Value&, const jsi::Value* args, size_t count) -> jsi::Value {\n${body}\n  });`,
      );
      ...
      if (statics.length) {
        ...
        for (const s of statics) {
          ...
          const b = this.callBody(
            fname,
            ps,
            ret,
            isAsync,
            `lucent_app::${c.cppName}::${cppIdent(memberName(s))}`,
          );
          lines.push(
            `    defineFunction(rt, ctor, ${q(memberName(s))}, ${ps.length}, [installed = host.shared_from_this()](jsi::Runtime& rt, const jsi::Value&, const jsi::Value* args, size_t count) -> jsi::Value {\n${b}\n    });`,
          );
        }
        lines.push("  }");
      }
    ...
    for (const c of m.consts) {
      ...
      lines.push(
        `  exports.setProperty(rt, ${q(name)}, Convert<${this.reg.cpp(c.type)}>::toJs(rt, host, ${c.cpp}));`,
      );
    }
    lines.push("}");
    return lines.join("\n");
  ...
  private unionConvert(u: LType & { k: "union" }): string {
    ...
    for (const m of u.ms) {
      ...
      switch (m.k) {
        ...
        case "class": {
          ...
          tests.push(
            `if (std::dynamic_pointer_cast<lucent_app::${info.cppName}>(instanceOf(rt, v))) ${ret}`,
          );
    ...
    if (objectMembers.length === 1) {
      tests.push(
        `if (v.isObject()) return ${s}(Convert<${this.reg.cpp(objectMembers[0]!)}>::fromJs(rt, v, p));`,
      );
  ...
  private discriminate(s: string, members: LType[]): string {
    ...
    for (const name of candidates) {
      ...
      if (values.every((v) => v !== undefined) && new Set(values).size === values.length) {
        const branches = structs.map(
          (_st, i) =>
            `if (d == ${q(values[i]!)}) return ${s}(Convert<${this.reg.cpp(members[i]!)}>::fromJs(rt, v, p));`,
        );
