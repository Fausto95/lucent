...
export function linkModule(
  ...
  const invalid = Object.entries(libraries).flatMap(([name, library]) =>
    validateLibrary(library).map((message) => diagnostic("LUCENT1006", { start: 0, end: 0 }, `${name}: ${message}`)),
  );
  if (invalid.length)
  ...
  const visit = (path: string): void => {
    ...
    if (active.has(path)) {
      diagnostics.push(
        diagnostic("LUCENT1006", { start: 0, end: 0 }, `Import cycle: ${[...active, path].join(" → ")}`),
      );
      return;
    }
    active.add(path);
      ...
      if (binding) {
        for (const name of Object.keys(binding.contract?.parameters ?? {}))
          if (!fn.params.some((p) => p.name === name))
            diagnostics.push(
              diagnostic("LUCENT1006", fn.span, `Unknown native contract parameter ${name} in ${fn.name}.`),
            );
        fn.binding = binding;
    ...
    for (const imp of module.imports) {
      ...
      if (imp.source === "@lucent-lang/core/events") {
        for (const b of imp.bindings ?? []) {
          if (b.imported === "Event" && b.typeOnly) types.set(b.local, "Event");
            ...
            diagnostics.push(diagnostic("LUCENT1006", imp.span, `Unknown event import ${b.imported}`));
        }
        continue;
      }
      if (imp.source === "@lucent-lang/core/ui") {
        for (const binding of imp.bindings ?? []) {
          ...
          else diagnostics.push(diagnostic("LUCENT1006", imp.span, `Unknown UI import ${binding.imported}`));
        }
        continue;
      }
        ...
        if (view && !binding.typeOnly) {
          ...
          for (const language of ["swift", "kotlin"] as const) {
            ...
            if (!descriptor || typeof descriptor.template !== "string") {
              diagnostics.push(
                diagnostic("LUCENT1006", imp.span, `Missing ${language} template for ${binding.imported}.`),
              );
              continue;
            }
              ...
              if (
                ...
                diagnostics.push(
                  diagnostic(
                    "LUCENT1006",
                    imp.span,
                    `Invalid or missing default for ${language} template token ${token}.`,
                  ),
                );
            }
          }
