...
export function sdkDts(schema: SdkModuleSchema): string {
    ...
    const base = ((): string => {
      switch (t.k) {
        ...
        case "ref": {
          ...
          const ps = sam.params
            .map((p, i) => `arg${i}: ${tsType(parseSdkType(p.type, t.module), true)}`)
            .join(", ");
  ...
  for (const type of schema.types) {
    if (type.kind === "enum") {
      body.push(
        `export declare enum ${type.name} {`,
        ...type.cases.map((c) => `  ${c.name} = ${c.value},`),
        "}",
        "",
      );
      continue;
    ...
    if (type.kind === "struct") {
      ...
      body.push(
        `export declare type ${type.name} = { ${type.fields.map((f) => `${f.name}: ${tsType(parseSdkType(f.type, schema.module))}`).join("; ")} };`,
        "",
      );
      continue;
    }
  ...
  for (const f of schema.functions ?? []) {
    const params = f.params
      .map((p) => `${safeName(p.name)}: ${tsType(parseSdkType(p.type, schema.module), false)}`)
      .join(", ");
    body.push(
      `export declare function ${f.name}(${params}): ${tsType(parseSdkType(f.returns, schema.module))};`,
    );
  }
  for (const c of schema.constants ?? [])
    body.push(`export declare const ${c.name}: ${tsType(parseSdkType(c.type, schema.module))};`);
  const head = [
    `// Generated by Lucent from the ${schema.platform} binding schema of ${schema.module}. Do not edit.`,
  ];
  ...
  for (const module of reexports(schema, [...imports.keys()]))
    head.push(`export * from "lucent:${module}";`);
...
function classDts(
  ...
  cls: SdkClassSchema,
  tsType: (t: SdkType, out?: boolean) => string,
  use: (module: string, name: string) => void,
): string[] {
  const parse = (s: string | SdkType, tps: readonly string[] = []) =>
    parseSdkType(s, schema.module, tps);
  const params = (ps: { name: string; type: string | SdkType }[], tps: readonly string[] = []) =>
    ps.map((p) => `${safeName(p.name)}: ${tsType(parse(p.type, tps), false)}`).join(", ");
  ...
  const ext = cls.extends
    ? ` extends ${tsType(parse(cls.extends))}`
    : schema.platform === "ios" && !cls.interface
      ? " extends NSObject"
      : "";
  ...
  out.push(
    `export declare ${cls.interface || cls.abstract ? "abstract " : ""}class ${cls.name}${ext} {`,
  );
  ...
  if (!cls.constructors?.length && !cls.inheritsInit && !cls.interface)
    out.push("  protected constructor();");
  for (const c of cls.constructors ?? [])
    out.push(`${memberDoc(c)}  constructor(${params(c.params)});`);
  ...
  for (const p of (cls.properties ?? []).filter((x) => !methodNames.has(`${!!x.static}:${x.name}`)))
    out.push(
      `${memberDoc(p)}  ${p.static ? "static " : ""}${p.readonly ? "readonly " : ""}${p.name}: ${tsType(parse(p.type))};`,
    );
  for (const m of cls.methods ?? []) {
    const tps = m.typeParams ?? [];
    // Optional protocol requirements, and Java interfaces' default methods, need no implementation.
    const optional =
      m.optional || (schema.platform === "android" && cls.interface && !m.abstract && !m.static);
    ...
    if (m.async) {
      ...
      out.push(
        `${promiseHead}(${params(m.params.slice(0, -1), tps)}): Promise<${tsType(parse(m.async.returns, tps))}>;`,
      );
    }
  }
  out.push("}");
  if (cls.implements?.length)
    out.push(
      `export declare interface ${cls.name} extends ${cls.implements.map((i) => tsType(parse(i))).join(", ")} {}`,
    );
  return out;
}
...
export function stubDts(platform: Platform, module: string, names: NamesIndex): string {
  const out = [
    `// Names only: import lucent:${platform}/${module} in a platform file to use these types' members. Generated by Lucent. Do not edit.`,
  ];
  ...
  for (const [name, t] of Object.entries(names.types).sort(([a], [b]) =>
    ...
    else if (t.kind === "struct" && t.fields)
      out.push(
        `export declare type ${name} = { ${t.fields.map((f) => `${f.name}: ${stubFieldType(parseSdkType(f.type, module))}`).join("; ")} };`,
      );
    else if (t.kind === "struct")
      out.push(`export declare type ${name} = { readonly __lucent_struct_${name}?: never };`);
    else if (t.kind === "protocol") out.push(`export declare abstract class ${name} {}`);
    else
      out.push(
        `export declare class ${name}${platform === "ios" ? " extends NSObject" : ""} {`,
        `  private readonly __lucent_${name}: never;`,
