...
describe("withLucentPaths", () => {
  ...
  it("adds compilerOptions when there are none", () => {
    expect(withLucentPaths('{ "extends": "expo/tsconfig.base" }')).toBe(
      `{ "extends": "expo/tsconfig.base", "compilerOptions": { "paths": { ${entry} } } }`,
    );
  });

  it("fills empty objects", () => {
    expect(withLucentPaths('{ "compilerOptions": { "paths": {} } }')).toBe(
      `{ "compilerOptions": { "paths": { ${entry} } } }`,
    );
  ...
  it("leaves a config that maps lucent:* alone", () => {
    expect(
      withLucentPaths(`{ "compilerOptions": { "paths": { "lucent:*": ["./types/*"] } } }`),
    ).toBeUndefined();
  });
