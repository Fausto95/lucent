...
function terminal() {
  const stream = Object.assign(new PassThrough(), { isTTY: true, columns: 100, rows: 30 }) as unknown as NodeJS.WriteStream;
  let written = "";
...
describe("live steps", () => {
  it("shows the running step, then keeps every finished step on screen", async () => {
    const t = terminal();
    const steps = liveSteps(createTheme({ color: false, interactive: true, unicode: true, links: false, width: 100 }), t.stream);
    ...
    steps.finish({ name: "package", label: "Native package", status: "ok", detail: ".lucent/native" });
    await steps.close();
