...
function lucent(root: string, env: Record<string, string>, ...args: string[]) {
  const r = spawnSync(process.execPath, [bin, "bench", ...args, "--root", root], { encoding: "utf8", env: { ...process.env, NO_COLOR: "1", ...env }, timeout: 600_000 });
  return { status: r.status, out: r.stdout + r.stderr, stdout: r.stdout };
}

describe("lucent bench", () => {
  it.skipIf(!hasHermes)("times each case natively and as JavaScript, with the same results", () => {
    const r = lucent(project(), {}, "--json");
    expect(r.status, r.out).toBe(0);
    const { cases } = JSON.parse(r.stdout) as { cases: { file: string; name: string; js: number; native: number; speedup: number; same: boolean }[] };
  ...
  it.skipIf(!hasHermes)("prints a speedup table", () => {
