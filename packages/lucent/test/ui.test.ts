...
describe("detectTerminal", () => {
  it("colours and animates an interactive terminal", () => {
    expect(detectTerminal({ TERM: "xterm-256color" }, tty, "darwin")).toMatchObject({ color: true, interactive: true, unicode: true, width: 120 });
  });

  it("is plain on a pipe", () => {
    expect(detectTerminal({ TERM: "xterm-256color" }, { isTTY: false }, "darwin")).toMatchObject({ color: false, interactive: false, width: 80 });
  });
  ...
  it("does not animate in CI", () => {
    expect(detectTerminal({ CI: "true" }, tty, "darwin")).toMatchObject({ interactive: false, color: true });
  });

  it("falls back to ASCII on a dumb terminal", () => {
    expect(detectTerminal({ TERM: "dumb" }, tty, "darwin")).toMatchObject({ color: false, interactive: false, unicode: false });
  });
  ...
  it("links only where the terminal renders OSC 8", () => {
    ...
    expect(detectTerminal({ TERM_PROGRAM: "iTerm.app" }, { isTTY: false }, "darwin").links).toBe(false);
  });
});

const plain: Terminal = { color: false, unicode: true, interactive: false, links: false, width: 80 };
...
describe("theme", () => {
  ...
  it("has ASCII symbols where Unicode is not safe", () => {
    ...
    expect(createTheme({ ...plain, unicode: false }).symbols).toMatchObject({ ok: "+", fail: "x", brand: "*" });
  });
});

describe("format", () => {
  it("writes durations as a person would", () => {
    expect([duration(41), duration(312), duration(1250), duration(65_000)]).toEqual(["41 ms", "312 ms", "1.3 s", "1 m 5 s"]);
  });

  it("aligns tables by visible width, colours included", () => {
    const t = createTheme(coloured);
    expect(table([[t.bold("module"), "ios"], ["haptics", t.success("●")]]).map((l) => l.replace(/\x1b\[[0-9;]*m/g, ""))).toEqual(["module   ios", "haptics  ●"]);
  });

  it("links with OSC 8 only when the terminal supports it", () => {
    expect(link("docs", "https://x.dev", plain)).toBe("docs");
    expect(link("docs", "https://x.dev", { ...coloured, links: true })).toBe("\x1b]8;;https://x.dev\x1b\\docs\x1b]8;;\x1b\\");
  });

  const source = "export async function share(url: string) {\n  const app = UIApplication.shared;\n  return app;\n}\n";

  it("frames code with the line before and after, and underlines the span", () => {
    expect(codeFrame(source, { line: 2, column: 15, length: 20 }, createTheme(plain))).toMatchSnapshot();
  });

  it("frames code in colour", () => {
    expect(codeFrame(source, { line: 2, column: 15, length: 20 }, createTheme(coloured))).toMatchSnapshot();
  });
