...
function lucent(root: string, ...args: string[]) {
  const r = spawnSync(process.execPath, [bin, "init", ...args, "--root", root], { encoding: "utf8", env: { ...process.env, NO_COLOR: "1" } });
...
function bareApp(): string {
  return write(fs.mkdtempSync(path.join(os.tmpdir(), "lucent-init-bare-")), {
    "package.json": JSON.stringify({ name: "Bare", dependencies: { react: "19.3.0", "react-native": "0.88.0" } }, null, 2),
    "package-lock.json": "{}\n",
...
function expoApp(): string {
  return write(fs.mkdtempSync(path.join(os.tmpdir(), "lucent-init-expo-")), {
    "package.json": JSON.stringify({ name: "expo-app", main: "expo-router/entry", dependencies: { expo: "~58.0.0", react: "19.3.0", "react-native": "0.88.0" } }, null, 2),
...
describe("init patches", () => {
  it("wraps the bare template's Metro config, once", () => {
    const text = "const {getDefaultConfig, mergeConfig} = require('@react-native/metro-config');\nconst config = {};\nmodule.exports = mergeConfig(getDefaultConfig(__dirname), config);\n";
    const wrapped = wrapMetro(text);
    expect(wrapped).toBe("const {getDefaultConfig, mergeConfig} = require('@react-native/metro-config');\nconst { withLucent } = require(\"@lucent-lang/lucent/metro\");\nconst config = {};\nmodule.exports = withLucent(mergeConfig(getDefaultConfig(__dirname), config));\n");
  ...
  it("applies the Gradle task after React Native's plugin, once", () => {
    ...
    expect(applied).toBe(`apply plugin: "com.android.application"\napply plugin: "com.facebook.react"\n${GRADLE_LINE}\n\nreact {}\n`);
  ...
  it("maps lucent:* and turns on noUncheckedIndexedAccess in tsconfig.json", () => {
    ...
    expect(JSON.parse(next!)).toEqual({ extends: "expo/tsconfig.base", compilerOptions: { strict: true, noUncheckedIndexedAccess: true, paths: { "lucent:*": ["./.lucent/native/types/*"] } } });
...
describe("lucent init --yes", () => {
  it("sets up a bare app, scaffolds a module and says what to run next", () => {
    ...
    expect(fs.readFileSync(path.join(root, "metro.config.js"), "utf8")).toContain("module.exports = withLucent(mergeConfig(getDefaultConfig(__dirname), config));");
    expect(fs.readFileSync(path.join(root, "android/app/build.gradle"), "utf8")).toContain(GRADLE_LINE);
    expect(fs.readFileSync(path.join(root, "react-native.config.js"), "utf8")).toContain('"lucent": { root:');
    expect(fs.readFileSync(path.join(root, ".gitignore"), "utf8")).toContain(".lucent/");
    expect(JSON.parse(fs.readFileSync(path.join(root, "tsconfig.json"), "utf8")).compilerOptions.paths).toEqual({ "lucent:*": ["./.lucent/native/types/*"] });
    expect(fs.readFileSync(path.join(root, "src/hello.lucent.ts"), "utf8")).toContain("export function hello(");
    expect(r.out.trimEnd().split("\n").at(-1)).toMatch(/^next +npx lucent build && npx react-native run-ios$/);
  });
  ...
  it("sets up an Expo app: the config plugin instead of Gradle", () => {
    ...
    expect(JSON.parse(fs.readFileSync(path.join(root, "app.json"), "utf8")).expo.plugins).toEqual(["expo-router", "@lucent-lang/lucent"]);
    expect(fs.readFileSync(path.join(root, "metro.config.js"), "utf8")).toBe('const { getDefaultConfig } = require("expo/metro-config");\nconst { withLucent } = require("@lucent-lang/lucent/metro");\n\nmodule.exports = withLucent(getDefaultConfig(__dirname));\n');
    expect(fs.existsSync(path.join(root, "android"))).toBe(false);
    expect(r.out.trimEnd().split("\n").at(-1)).toMatch(/^next +pnpm exec expo run:ios$/);
  });

  for (const [kind, make] of [["bare", bareApp], ["Expo", expoApp]] as const) {
  ...
  it("leaves an app.config.js alone and says what to add", () => {
    ...
    const r = lucent(root, "--yes");
    expect(fs.readFileSync(path.join(root, "app.config.js"), "utf8")).toBe("export default { name: 'a' };\n");
...
describe("lucent init without a terminal", () => {
  it("shows the changes as diffs and applies nothing without --yes", () => {
    ...
    expect(r.out).toMatch(/metro\.config\.js[\s\S]*- module\.exports = mergeConfig\(getDefaultConfig\(__dirname\), config\);\n[\s\S]*\+ module\.exports = withLucent\(/);
...
describe("lucent init in a terminal", () => {
  it("shows each change as a diff and applies the ones confirmed", async () => {
    ...
    const theme = createTheme({ color: false, interactive: true, unicode: true, links: false, width: 80 });
    const changes = [
      { file: "metro.config.js", why: "bundle *.lucent.ts as native proxies", before: "module.exports = config;\n", after: "module.exports = withLucent(config);\n" },
      { file: ".gitignore", why: "ignore the generated native package", before: "node_modules/\n", after: "node_modules/\n.lucent/\n" },
    ];
    let answers: boolean[] | undefined;
    const ui = render(createElement(Confirm, { changes, theme, onDone: (a: boolean[]) => (answers = a) }));
    const tick = () => new Promise((r) => setTimeout(r, 30));
    await tick();
    expect(ui.lastFrame()).toMatch(/metro\.config\.js[\s\S]*- module\.exports = config;[\s\S]*\+ module\.exports = withLucent\(config\);[\s\S]*Apply\?/);
    ui.stdin.write("y");
    await tick();
