...
const specs: CommandSpec[] = [
  {
    ...
    summary: "Build",
    flags: [
      { name: "force", description: "Rebuild" },
      { name: "platforms", value: "list", description: "Targets" },
    ],
    load: noop,
  },
  {
    name: "sdk prefetch",
    summary: "Prefetch",
    flags: [{ name: "ios", value: "modules", description: "iOS modules" }],
    load: noop,
  },
];
...
describe("parseArgs", () => {
  it("finds the command, its flags and positionals", () => {
    ...
    expect(r).toMatchObject({
      command: { name: "build" },
      flags: { force: true, platforms: "ios,android" },
      positionals: ["extra"],
    });
  });

  it("matches commands of two words", () => {
    expect(parseArgs(["sdk", "prefetch", "--ios", "UIKit"], specs)).toMatchObject({
      command: { name: "sdk prefetch" },
      flags: { ios: "UIKit" },
    });
  });
  ...
  it("accepts --flag=value and the global flags", () => {
    expect(
      parseArgs(["build", "--platforms=ios", "--json", "--root", "/app"], specs),
    ).toMatchObject({ flags: { platforms: "ios", json: true, root: "/app" } });
  });

  it("reports unknown flags and missing values with the command", () => {
    expect(parseArgs(["build", "--nope"], specs)).toEqual({
      error: "unknown flag --nope for lucent build",
    });
    expect(parseArgs(["build", "--platforms"], specs)).toEqual({
      error: "--platforms needs a value (--platforms <list>)",
    });
  });

  it("reports unknown commands, suggesting the closest", () => {
    expect(parseArgs(["biuld"], specs)).toEqual({
  ...
  it("leaves a bare invocation and --help to the caller", () => {
    ...
    expect(parseArgs(["build", "--help"], specs)).toMatchObject({
      command: { name: "build" },
      flags: { help: true },
    });
  });
