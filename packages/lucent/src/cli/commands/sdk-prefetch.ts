...
export async function run({ root, flags, out }: Invocation): Promise<number> {
  ...
  const listed = (p: string) =>
    typeof flags[p] === "string" ? (flags[p] as string).split(",").filter(Boolean) : undefined;
  let wanted: { ios?: string[]; android?: string[] } = {
    ios: listed("ios"),
    android: listed("android"),
  };
  ...
  const steps: Steps = out.json
    ? plainSteps(() => {}, t)
    : out.terminal.interactive
      ? (await import("../ui/live-steps.tsx")).liveSteps(t)
      : plainSteps((l) => out.print(l), t);
  let extracted = 0;
  let cached = 0;
  try {
  ...
  if (out.json) {
    out.data({
      ok: failed === 0,
      modules: steps.results.map((s) => ({
        module: s.name,
        status: s.status,
        ...(s.ms !== undefined ? { ms: s.ms } : {}),
        ...(s.detail ? { error: s.detail } : {}),
      })),
    });
    return failed ? 1 : 0;
  }
