...
export async function buildOrCheck(command: "build" | "check", { root, flags, out }: Invocation): Promise<number> {
  ...
  try {
    result = await buildProject(root, { mode: command, force: !!flags.force, platforms: typeof flags.platforms === "string" && flags.platforms ? (flags.platforms.split(",") as Target[]) : undefined, out: typeof flags.out === "string" ? flags.out : undefined, prefetch: true }, steps, notify);
  } finally {
    await steps.close();
  }
  ...
  if (out.json) {
    ...
    else if (command === "check") out.data({ ok: result.ok, modules: result.modules.map((m) => m.name), diagnostics: result.diagnostics, errors: result.diagnostics.length, ms: result.ms });
    else out.data({ ok: result.ok, upToDate: result.upToDate, out: path.resolve(root, typeof flags.out === "string" ? flags.out : ".lucent/native"), modules: result.modules, steps: steps.results, diagnostics: result.diagnostics, notices, next: result.next, ms: result.ms });
  ...
  if (command === "check") {
    out.print(`${theme.success(theme.symbols.ok)} ${plural(result.modules.length, "module")}, no problems  ${theme.dim(result.upToDate ? `${duration(result.ms)} (unchanged since the last check)` : duration(result.ms))}`);
    return 0;
  }
  if (result.upToDate) return 0;
  out.print("");
  for (const line of table(result.modules.map((m, i) => [i ? "" : theme.dim("modules"), m.name, m.platforms.join(" ")]))) out.print(line);
