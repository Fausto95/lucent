...
export function run({ positionals, out }: Invocation): number {
  ...
  if (!wanted) {
    for (const line of table(Object.entries(Explanations).map(([code, e]) => [t.bold(code), e.title]))) out.print(line);
  ...
  for (const [label, files, paint] of [["wrong", e.wrong, t.error], ["right", e.right, t.success]] as const) {
    for (const [file, source] of Object.entries(files)) out.print(`${paint(`${label === "wrong" ? t.symbols.fail : t.symbols.ok} ${label}`)}  ${t.dim(file)}\n${indent(source)}\n`);
  }
