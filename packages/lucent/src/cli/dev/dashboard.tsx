...
export async function dashboard(options: DashboardOptions): Promise<void> {
  ...
  stdout.write(ENTER);
  const app = render(<Dashboard {...options} />, { stdout, stdin: options.stdin ?? process.stdin, exitOnCtrlC: false, patchConsole: false });
...
function Dashboard({ session, theme: t, root, open, doctor }: DashboardOptions) {
  ...
  const cell = (state: PlatformState) => (state === "ok" ? t.success(t.symbols.on) : state === "error" ? t.error(t.symbols.fail) : state === "building" ? t.progress(t.symbols.off) : " ");
  const last = s.lastBuild;
  const when = s.building ? t.progress(`building${t.symbols.ellipsis}`) : last ? `${last.at.toLocaleTimeString("en-GB")}  ${duration(last.ms)}` : "";
  const rows = table([[t.dim("MODULE"), t.dim("IOS"), t.dim("ANDROID"), t.dim("LAST BUILD")], ...s.modules.map((m) => [m.name, cell(m.platforms.ios), cell(m.platforms.android), when])], 3);
  const status = last?.fatal ? t.error(`${t.symbols.fail} ${last.fatal}`) : last?.ok && last.next?.rebuild ? t.warn(`next: ${nextText(last.next)}`) : "";
  const rule = (title: string) => t.dim(`${t.symbols.rule} ${title} ${t.symbols.rule.repeat(Math.max(3, t.terminal.width - title.length - 4))}`);

  return (
    <Box flexDirection="column">
      <Text>{`${t.brand(t.symbols.brand)} ${t.bold("lucent dev")}   ${t.dim(`watching ${s.watching.join(" · ") || root}`)}`}</Text>
      <Text> </Text>
      {s.modules.length ? <Text>{rows.join("\n")}</Text> : <Text>{t.dim(s.building ? `building${t.symbols.ellipsis}` : "no *.lucent.ts modules yet")}</Text>}
      {status ? <Text>{`\n${status}`}</Text> : null}
      {s.problems.length ? (
        <Box flexDirection="column" marginTop={1}>
          <Text>{rule(`problems (${s.problems.length})`)}</Text>
          {s.problems.map((d, i) => (
            <Text key={i}>{`${d === problem ? t.brand("›") : " "} ${d.file ? `${d.file}${d.line ? `:${d.line}` : ""}  ` : ""}${t.bold(d.code)}  ${d.message}`}</Text>
          ))}
          {problem?.fix ? <Text>{`  ${t.success("fix")}  ${problem.fix}`}</Text> : null}
          {problem?.source !== undefined && problem.line ? <Text>{codeFrame(problem.source, { line: problem.line, column: problem.column ?? 1, length: problem.length ?? 1 }, t)}</Text> : null}
        </Box>
      ) : null}
      {checks ? (
        <Box flexDirection="column" marginTop={1}>
          <Text>{rule("doctor")}</Text>
          {checks.filter((c) => c.status !== "skip").map((c, i) => (
            <Text key={i}>{`${c.status === "ok" ? t.success(t.symbols.ok) : c.status === "fail" ? t.error(t.symbols.fail) : t.warn(t.symbols.warn)} ${c.label}  ${t.dim(c.detail)}${c.fix ? `\n    fix: ${c.fix}` : ""}`}</Text>
          ))}
        </Box>
      ) : null}
