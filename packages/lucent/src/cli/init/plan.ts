...
const first = (root: string, files: string[]) =>
  files.find((f) => fs.existsSync(path.join(root, f)));
...
export function planInit(root: string): InitPlan {
  ...
  if (kind === "expo") {
    ...
    if (fs.existsSync(path.join(root, "app.json")))
      change("app.json", pluginWhy, addExpoPlugin(read(root, "app.json")!));
    else {
      const config = first(root, ["app.config.ts", "app.config.js"]);
      if (config && !read(root, config)!.includes("@lucent-lang/lucent"))
        manual.push({ file: config, why: pluginWhy, snippet: 'plugins: ["@lucent-lang/lucent"]' });
    }
  } else {
    ...
    if (linked === "manual")
      manual.push({ file: "react-native.config.js", why: linkWhy, snippet: RN_CONFIG_SNIPPET });
