...
export async function buildProject(
  ...
  if (!platforms && files.some((f) => platformOf(f) || usesPlatforms(f))) {
    ...
    const installed = (["ios", "android"] as const).filter(
      (p) => sdkAvailable(p, sdk) && !(p === "android" && deferAndroid),
    );
    for (const p of ["ios", "android"] as const) {
      ...
      notify({
        level: "warn",
        text: `${"missing" in why ? why.missing : `no ${p} SDK`}; skipped ${p === "ios" ? "iOS" : "Android"} (build it with --platforms ${p} once the SDK is installed)`,
      });
    }
  ...
  const key =
    ...
    `:${createHash("sha256").update(JSON.stringify(native)).digest("hex").slice(0, 12)}`;
  ...
  const diagnostics = result.diagnostics.map((d) => ({
    ...d,
    file: d.file && path.relative(root, d.file),
  }));
  if (!result.ok) {
    steps.finish({
      name: "check",
