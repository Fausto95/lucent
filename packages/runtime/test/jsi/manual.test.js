...
function eq(label, actual, expected) {
  ...
  if (a === e) { __passes++; } else { __failures++; print("FAIL " + label + ": got " + a + ", expected " + e); }
}
function throws(label, fn, check) {
  try { fn(); __failures++; print("FAIL " + label + ": did not throw"); }
  catch (e) { if (check(e)) __passes++; else { __failures++; print("FAIL " + label + ": wrong error " + e + " code=" + e.code); } }
}
function lucentClass(factory) {
  function C() { return factory.apply(undefined, arguments); }
...
throws("hash bad arg", function () { m.hash(42); }, function (e) {
  return e instanceof TypeError && e.message === "hash: argument 'input' must be a string, got a number";
});
...
var seen = [];
m.each([1, 2, 3], function (v) { seen.push(v * 10); });
eq("sync callback", seen, [10, 20, 30]);
eq("sync callback with result", m.sumMapped([1, 2, 3], function (v) { return v * v; }), 14);
throws("callback throws", function () { m.sumMapped([1], function () { throw new RangeError("inner"); }); }, function (e) {
...
m.progress(3, function (i) { steps.push(i); }).then(function (n) {
  eq("async callback steps", steps, [1, 2, 3]);
  eq("async return", n, 3);
});
m.askJs(function (q) { return Promise.resolve("JS answered " + q); }).then(function (r) {
  eq("await JS promise", r, "got JS answered name?");
});
m.askJs(function () { return Promise.reject(new Error("no")); }).then(
  function () { __failures++; print("FAIL: should reject"); },
  function (e) { eq("rejection passes through", e.message, "no"); }
);
setTimeout(function () {
