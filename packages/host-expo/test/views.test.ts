...
test("namespaces native view events to avoid React Native bubbling event collisions", () => {
  const module = compile('import {Button,type NativeView} from "@lucent-lang/ui"; import type {Event} from "@lucent-lang/events"; type Props={onPress:Event<void>}; export function Card(props:Props):NativeView{return <Button title="Go" onPress={props.onPress}/>;}', {fileName:"card.lucent.tsx"}).module!;
  ...
  expect(tree.get("android/src/main/java/expo/modules/lucent/LucentCardCardView.kt")).toContain('Events("onLucentCardCardView_onPress")');
