...
describe.skipIf(!xcode)("iOS extractor", () => {
  ...
  it("maps initializers, factories and class properties to their selectors", () => {
    ...
    expect(method("named")[0]).toMatchObject({ static: true, selector: "widgetNamed:", params: [{ name: "name", type: T("string") }], returns: T("Widgets.WDGWidget") });
    const props = Object.fromEntries(widget().properties!.map((p) => [p.name, p]));
    expect(props.shared).toMatchObject({ static: true, readonly: true, selector: "sharedWidget", type: T("Widgets.WDGWidget") });
    ...
    expect(props.isEnabled).toMatchObject({ readonly: true, selector: "isEnabled", type: T("bool") });
  ...
  it("types parameters and results: nullability, collections, data, dates, id", () => {
    expect(method("touch")[0]).toMatchObject({ selector: "touch:other:", params: [{ type: T("Widgets.WDGShape") }, { type: T("Widgets.WDGWidget?") }], returns: T("void") });
    ...
    expect(method("setObject")[0]!.params.map((p) => p.type)).toEqual(["id", "string"].map((x) => T(x)));
  ...
  it("turns NSError** into throws", () => {
    ...
    expect(method("save")[0]).toMatchObject({ selector: "saveToPath:error:", throws: true, params: [{ name: "path", type: T("string") }], returns: T("void") });
  });

  it("keeps NSError** out-parameters where Swift does (NS_SWIFT_NOTHROW)", () => {
    expect(method("canFrob")[0]).toMatchObject({ selector: "canFrob:error:", params: [{ name: "level", type: T("NSInteger") }, { name: "error", type: T("Out<error>?") }], returns: T("bool") });
  ...
  it("records availability, C functions and constants, and what it skips", () => {
    ...
    expect(mod().constants).toEqual(expect.arrayContaining([{ name: "WDGVersionString", type: T("string") }]));
  ...
  it("types blocks as functions: whether they escape, and the thread they run on", () => {
    ...
    expect(on("observe")).toMatchObject({ selector: "observeWithBlock:", params: [{ name: "block", type: T("@escaping (string, NSInteger) => void") }], returns: T("void") });
    expect(on("onDone")).toMatchObject({ selector: "onDone:", params: [{ name: "done", type: T("@escaping () => void") }] });
    ...
    expect(method("countWhere")[0]).toMatchObject({ selector: "countWhere:", params: [{ name: "predicate", type: T("@main (string) => bool") }], returns: T("NSInteger") });
    expect(method("animate")[0]).toMatchObject({ selector: "animate:completion:", params: [{ name: "changes", type: T("@escaping @main () => void") }, { name: "completion", type: T("(@main (bool) => void)?") }] });
  });
  ...
  it("turns completion handlers into promises where Swift imports them as async", () => {
    ...
    if (loader.kind !== "class") throw new Error("not a class");
    const on = (name: string) => loader.methods!.find((m) => m.name === name)!;
    expect(method("fetch")[0]).toMatchObject({ selector: "fetchWithCompletion:", params: [{ name: "completion", type: T("@escaping @main (bool) => void") }], async: { returns: T("bool") } });
    expect(on("load")).toMatchObject({ selector: "loadWithReply:", params: [{ name: "reply", type: T("@escaping (NSData?, error?) => void") }], async: { returns: T("NSData"), throws: true } });
    expect(method("animate")[0]).toMatchObject({ async: { returns: T("bool") } });
    // Swift names the async form itself: get… drops its prefix.
    expect(on("getItemsWithCompletionHandler")).toMatchObject({ selector: "getItemsWithCompletionHandler:", async: { returns: T("string[]"), name: "items" } });
  ...
  it("names protocol requirements from their own Swift names, and marks optional ones", () => {
    ...
    expect(d.methods).toMatchObject([
      { name: "loader_didLoad", selector: "loader:didLoadData:", params: [{ name: "loader", type: T("Widgets.WDGLoader") }, { name: "data", type: T("NSData") }], returns: T("void") },
      { name: "loader_didFailWithError", selector: "loader:didFailWithError:", params: [{ name: "loader", type: T("Widgets.WDGLoader") }, { name: "error", type: T("error") }], returns: T("void"), optional: true },
      { name: "loaderShouldRetry", selector: "loaderShouldRetry:", params: [{ name: "loader", type: T("Widgets.WDGLoader") }], returns: T("bool"), optional: true },
    ]);
  ...
  it("marks weak properties, which do not keep their value alive", () => {
    const loader = type("WDGLoader");
    if (loader.kind !== "class") throw new Error("not a class");
    expect(loader.properties!.find((p) => p.name === "delegate")).toMatchObject({ type: T("Widgets.WDGLoaderDelegate?"), setter: "setDelegate:", weak: true });
  });

  it("reads C structs of numbers and structs, by value", () => {
    expect(type("WDGPoint")).toEqual({ kind: "struct", name: "WDGPoint", native: "WDGPoint", fields: [{ name: "x", type: T("double") }, { name: "y", type: T("double") }] });
    expect(type("WDGCircle")).toEqual({ kind: "struct", name: "WDGCircle", native: "WDGCircle", fields: [{ name: "center", type: T("Widgets.WDGPoint") }, { name: "radius", type: T("double") }] });
    expect(method("origin")[0]).toMatchObject({ selector: "origin", returns: T("Widgets.WDGPoint") });
    expect(widget().properties!.find((p) => p.name === "circle")).toMatchObject({ type: T("Widgets.WDGCircle"), setter: "setCircle:" });
  });
  ...
  it("reads anonymous C enums (typedef enum {…} name_t), and C functions' escaping blocks", () => {
    ...
    expect(mod().functions!.find((f) => f.name === "WDGWatch")).toEqual({ name: "WDGWatch", params: [{ name: "handler", type: T("@escaping (Widgets.wdg_state_t) => void") }], returns: T("void") });
  });

  it("reads typed string keys (NS_TYPED_ENUM) as string constants of their C globals", () => {
    expect(type("WDGKey")).toMatchObject({ kind: "class", properties: [{ name: "name", static: true, readonly: true, type: T("string"), global: "WDGKeyName" }] });
  });

  it("bridges CoreFoundation types, and out-pointers of C functions", () => {
    expect(mod().constants).toEqual(expect.arrayContaining([{ name: "WDGKeyClass", type: T("CFString") }]));
    const fn = (name: string) => mod().functions!.find((f) => f.name === name);
    expect(fn("WDGItemCopy")).toEqual({ name: "WDGItemCopy", params: [{ name: "query", type: T("CFDictionary") }, { name: "result", type: T("Out<CFTypeRef>?") }], returns: T("int32") });
    expect(fn("WDGCopyData")).toEqual({ name: "WDGCopyData", params: [{ name: "name", type: T("CFString") }], returns: T("CFData?") });
  });
});
