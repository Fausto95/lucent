...
function locateAndroid(opts: SdkOptions): Located | { missing: string } {
  ...
  if (classpath && fs.existsSync(classpath)) {
    const cp = JSON.parse(fs.readFileSync(classpath, "utf8")) as {
      jars?: string[];
      aars?: string[];
    };
  ...
  return {
    dir: path.join(cacheRoot(opts), "sdk/android", key),
    describe,
    android: { jars: all, apiVersions, annotations, dependencies: dependencies.length, classpath },
  };
}
...
function extractAndroidModule(sdk: Located, module: string): SdkLookup {
  ...
  if (!index.packages.has(module)) {
    ...
    if (dependencies)
      where += ` and ${dependencies} dependency jar${dependencies === 1 ? "" : "s"} from ${classpath}`;
    else if (classpath)
...
function locateIos(opts: SdkOptions): Located | { missing: string } {
  ...
  for (const f of fs.existsSync(frameworks) ? fs.readdirSync(frameworks) : [])
    if (f.endsWith(".framework")) modules.set(f.slice(0, -".framework".length), undefined);
  ...
  const podsDir = lockfile
    ? `${fs.realpathSync(path.join(path.dirname(lockfile), "Pods"))}${path.sep}`
    : undefined;
  ...
  const lock =
    lockfile && fs.existsSync(lockfile) ? [hash([fs.readFileSync(lockfile, "utf8")])] : [];
  const key = `iphonesimulator${version}-${build}-${hash([extractorVersion(), ...includePaths, ...frameworkPaths, ...lock, ...identities])}`;
  return {
    dir: path.join(cacheRoot(opts), "sdk/ios", key),
    describe: `iOS ${version} SDK (${build})`,
    ios: {
      sdk,
      ios: { modules: [], includePaths, frameworkPaths, moduleMaps, xcrun },
      frameworks,
      modules,
      umbrellas,
    },
...
function headerIndex(sdk: Located): Record<string, string> {
  ...
  for (const [module, headers] of sdk.ios!.modules) {
    ...
    const files = headers
      ? [...new Set(headers.flatMap((h) => [h, ...filesUnder(path.dirname(h), /\.h$/)]))]
      : filesUnder(path.join(sdk.ios!.frameworks, `${module}.framework/Headers`), /\.h$/);
    scan(module, files);
  }
  write(file, index);
  return index;
...
function readGraph(dir: string, module: string): SymbolGraph | undefined {
  ...
  return fs.existsSync(file)
    ? (JSON.parse(fs.readFileSync(file, "utf8")) as SymbolGraph)
    : undefined;
}
...
function namesFor(sdk: Located, modules: string[]): NamesIndex[] {
  ...
  return modules
    .filter((m) => fs.existsSync(file(m)))
    .map((m) => JSON.parse(fs.readFileSync(file(m), "utf8")) as NamesIndex);
}
...
function extractIosModule(sdk: Located, module: string): SdkLookup {
  ...
  const deps = [
    ...new Set(
      [...externalUsrs(g)]
        .map((u) => ownerOf(u, owners))
        .filter((m): m is string => !!m && m !== module && modules.has(m)),
    ),
  ];
  ...
  return {
    schema: {
      ...
      header: umbrella
        ...
        : `${module}/${frameworkUmbrella(sdk.ios!.frameworks, module)}`,
...
export function sdkModule(platform: Platform, module: string, opts: SdkOptions = {}): SdkLookup {
  ...
  if (!("dir" in sdk)) result = sdk;
    ...
    const read = (): SdkLookup | undefined =>
      fs.existsSync(file)
        ? { schema: JSON.parse(fs.readFileSync(file, "utf8")) as SdkModuleSchema }
        : undefined;
    result = read() ??
      withLock(
        file,
        ...
        () => {
          const r =
            platform === "android"
              ? extractAndroidModule(sdk, module)
              : extractIosModule(sdk, module);
          if ("schema" in r) write(file, r.schema);
          return r;
        },
      ) ??
      read() ?? { missing: `lucent:${platform}/${module} could not be extracted` };
  }
...
export function cachedModules(
  ...
  const names = files
    .filter((f) => f.endsWith(".names.json"))
    .map((f) => f.slice(0, -".names.json".length))
    .filter((m) => !schemas.includes(m));
...
export function sdkModules(
  ...
  if (!("dir" in sdk)) return sdk;
  if (platform === "android")
    return [...jarIndex(sdk.android!.jars, sdk.android!.apiVersions).packages].sort();
