...
test('extracts labeled Swift functions and emits type declarations and native bindings',()=>{
 const schema=extractSwiftInterface('public func distance(from start: Swift.Double, to end: Swift.Double) throws -> Swift.Double','Geometry');
...
test('extracts Java static methods with primitive conversions',()=>{
 const schema=extractJavaSignatures('public final class java.lang.Math {\n public static double hypot(double, double);\n public static int abs(int);\n public java.lang.String toString();\n}');
 const output=generateBindingLibrary(schema);
...
test('reports unsupported SDK signatures instead of emitting misleading declarations',()=>{
 const schema=extractSwiftInterface('public func open(_ callback: @escaping () -> Void)\npublic func read() -> Foundation.URL','Foundation');
