...
test("emits native binding bodies and dispatcher hops", () => {
  const result = compile('import { now } from "@lucent-lang/platform/clock"; /** @thread main */ export async function f(): Promise<number> { return now(); }', {fileName: "clock.lucent.ts"});
