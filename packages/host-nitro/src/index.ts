...
function emitPackage(modules: IRModule[], options: EmitOptions): FileTree {
  ...
  emitNativeSidecars(
    ...
    ANDROID_PACKAGE,
    "NitroModules",
    "com.margelo.nitro.core.ArrayBuffer",
  );
...
function podspec(): string {
  return fillNative(nativeAsset("NitroLucent.podspec"), { moduleName: IOS_MODULE_NAME });
}
...
function buildGradle(): string {
  return fillNative(nativeAsset("build.gradle"), {
    moduleName: IOS_MODULE_NAME,
    androidPackage: ANDROID_PACKAGE,
  });
}

const CMAKE = fillNative(nativeAsset("CMakeLists.txt"), { moduleName: IOS_MODULE_NAME });

const CPP_ADAPTER = fillNative(nativeAsset("cpp-adapter.cpp"), {
  moduleName: IOS_MODULE_NAME,
  namespace: ANDROID_NAMESPACE,
});

const PACKAGE_KT = (ident: string, views: string[]): string =>
  GENERATED_HEADER +
  "\n" +
  fillNative(nativeAsset("LucentPackage.kt"), {
    ident,
    androidPackage: ANDROID_PACKAGE,
    moduleName: IOS_MODULE_NAME,
    viewImports: views.length
      ? "\n" + views.map((name) => `import ${ANDROID_PACKAGE}.views.Hybrid${name}Manager`).join("\n")
      : "",
    viewManagers: views.length
      ? `  override fun createViewManagers(reactContext: ReactApplicationContext): List<com.facebook.react.uimanager.ViewManager<*, *>> = listOf(${views
          .map((n) => `Hybrid${n}Manager()`)
          .join(", ")})\n\n`
      : "",
  });
