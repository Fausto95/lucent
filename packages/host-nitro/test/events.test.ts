...
test("bridges typed native event subscriptions", () => {
  const module = compile('import {event} from "@lucent-lang/events"; export const progress=event<number>(); export function run():void { progress.emit(50); }', {fileName:"progress.lucent.ts"}).module!;
